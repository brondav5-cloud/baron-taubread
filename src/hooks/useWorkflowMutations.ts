...
import type { CreateWorkflowInput } from "@/context/workflow/workflow.types";

const generateStepId = () =>
  `step_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;

function updateStepStatuses(steps: WorkflowStep[]): WorkflowStep[] {
...
export function useWorkflowMutations(
  ...
  setWorkflows: Dispatch<SetStateAction<WorkflowTask[]>>,
) {
      ...
      setWorkflows((prev) =>
        prev.map((wf) => {
          ...
          const updated = {
            ...wf,
            steps,
            status: (allCompleted ? "awaiting_approval" : "active") as WorkflowTask["status"],
            updatedAt: now,
            history: [
              ...wf.history,
              {
      ...
      setWorkflows((prev) =>
        prev.map((wf) => {
          ...
          const steps = wf.steps.map((step) => {
            ...
            return { ...step, checklist: [...checklist, { id: generateStepId(), text, completed: false }] };
          });
  ...
  const toggleChecklistItem = useCallback(
    (workflowId: string, stepId: string, itemId: string, userId: string, userName: string) => {
      const now = new Date().toISOString();
      setWorkflows((prev) =>
        prev.map((wf) => {
          if (wf.id !== workflowId) return wf;
    ...
    (workflowId: string, stepId: string, itemId: string) => {
      ...
      setWorkflows((prev) =>
        prev.map((wf) => {
          ...
          const steps = wf.steps.map((step) => {
            if (step.id !== stepId) return step;
            return { ...step, checklist: (step.checklist || []).filter((item) => item.id !== itemId) };
          });
          const updated = { ...wf, steps, updatedAt: now };
          if (companyId) {
            const u = workflowToDbWorkflow(updated, companyId);
  ...
  const addStepComment = useCallback(
    (workflowId: string, stepId: string, userId: string, userName: string, text: string) => {
      const now = new Date().toISOString();
      setWorkflows((prev) =>
        prev.map((wf) => {
          if (wf.id !== workflowId) return wf;
          const steps = wf.steps.map((step) => {
            ...
            return {
              ...step,
              comments: [...comments, { id: generateStepId(), userId, userName, text, createdAt: now }],
            };
          });
          const updated = {
            ...wf,
            ...
            history: [
              ...wf.history,
              { id: generateStepId(), action: "comment" as const, userId, userName, timestamp: now, details: `הוסיף הערה לשלב` },
            ],
          };
          if (companyId) {
            const u = workflowToDbWorkflow(updated, companyId);
            updateWorkflow(workflowId, { steps: u.steps, history: u.history, updated_at: now });
          }
          return updated;
        }),
      );
  ...
  const returnStep = useCallback(
    (workflowId: string, stepId: string, userId: string, userName: string, reason: string) => {
      const now = new Date().toISOString();
      setWorkflows((prev) =>
        prev.map((wf) => {
          if (wf.id !== workflowId) return wf;
          ...
          const steps = wf.steps.map((step, index) => {
            if (index === stepIndex - 1) {
              return { ...step, status: "active" as const, returnedBy: userId, returnedByName: userName, returnedAt: now, returnReason: reason, wasReturned: true };
            }
            if (index === stepIndex) {
              return { ...step, status: "pending" as const };
            }
          ...
          const updated = {
            ...
            history: [
              ...wf.history,
              { id: generateStepId(), action: "rejected" as const, userId, userName, timestamp: now, details: `החזיר שלב: ${reason}` },
            ],
          };
          if (companyId) {
            const u = workflowToDbWorkflow(updated, companyId);
            updateWorkflow(workflowId, { steps: u.steps, status: u.status, history: u.history, updated_at: now });
          }
          return updated;
        }),
      );
  ...
  const addStepAssignee = useCallback(
    (workflowId: string, stepId: string, userId: string, userName: string, byUserId: string, byUserName: string) => {
      const now = new Date().toISOString();
      setWorkflows((prev) =>
        prev.map((wf) => {
          if (wf.id !== workflowId) return wf;
          const steps = wf.steps.map((step) => {
            ...
            return { ...step, assignees: [...step.assignees, { userId, userName }] };
          });
          const updated = {
            ...wf,
            steps,
  ...
  const removeStepAssignee = useCallback(
    (workflowId: string, stepId: string, userId: string, byUserId: string, byUserName: string) => {
      const now = new Date().toISOString();
      setWorkflows((prev) =>
        prev.map((wf) => {
          if (wf.id !== workflowId) return wf;
          const steps = wf.steps.map((step) => {
            ...
            return { ...step, assignees: step.assignees.filter((a) => a.userId !== userId) };
          });
          const removedUserName = wf.steps.find((s) => s.id === stepId)?.assignees.find((a) => a.userId === userId)?.userName;
          const updated = {
            ...wf,
            steps,
            updatedAt: now,
