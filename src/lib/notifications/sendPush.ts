...
export async function sendPushToUsers(payload: NotificationPayload): Promise<{
  ...
  // Process all recipients in parallel for speed
  const results = await Promise.all(
    payload.recipientUserIds.map(async (recipientId) => {
      let rSent = 0, rFailed = 0, rEmails = 0, rSms = 0;
      try {
        ...
        // Log notification — wrapped in try so a DB error doesn't kill email/SMS
        try {
          ...
          console.error("[sendPushToUsers] notifications insert failed for", recipientId, dbErr);
        }
        ...
        if (payload.sendEmail && userRow?.email) {
          const fullUrl = payload.url ? `${baseUrl}${payload.url}` : `${baseUrl}/dashboard`;
          ...
          if (emailOk) rEmails++;
          else console.warn("[sendPushToUsers] email failed for", userRow.email);
        }
        ...
        if (payload.sendSms && userRow?.phone) {
          ...
          if (smsOk) rSms++;
          else console.warn("[sendPushToUsers] SMS failed for phone", userRow?.phone);
        }
      } catch (err) {
        console.error("[sendPushToUsers] unexpected error for recipientId", recipientId, err);
      }
      return { rSent, rFailed, rEmails, rSms };
    }),
  );
