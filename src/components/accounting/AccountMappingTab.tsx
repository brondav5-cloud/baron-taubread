...
interface Props {
  ...
  transactions?: { account_id: string; transaction_date: string; group_code: string; counter_account: string | null; debit?: number; credit?: number }[];
...
function SuppliersTab({
  ...
  // Per-account: total amounts and year groupings
  const accountStats = useMemo(() => {
    const amountMap = new Map<string, number>(); // accountId → total abs amount
    const yearData = new Map<string, Map<number, Set<string>>>(); // accountId → year → group_codes
    for (const tx of transactions) {
