...
import { Plus, Trash2, Loader2, Check, Zap, X, AlertCircle } from "lucide-react";
...
export function TransactionSplitsPanel({ txId, txAmount, txIsDebit, categories, docRows, onSaved }: Props) {
  ...
  // After-save: prompt to create rules
  const [saveRulesPrompt, setSaveRulesPrompt] = useState<
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      // Build list of classified rows that could become rules
      // Only rows that have both description AND category_id
      const classifiedRows = validRows
        .filter((r) => r.description.trim() && r.category_id);

      if (classifiedRows.length > 0) {
        // Check which ones don't yet have a rule saved
        const catMap = new Map(categories.map((c) => [c.id, c.name]));
        const prompt = classifiedRows.map((r) => ({
          description: r.description.trim(),
          category_id: r.category_id,
          category_name: catMap.get(r.category_id) ?? "",
        }));
        setSaveRulesPrompt(prompt);
      }
    } finally {
      setSaving(false);
    }
  ...
  const handleSaveRules = useCallback(async () => {
    if (saveRulesPrompt.length === 0) return;
    setSavingRules(true);
    try {
      const res = await fetch("/api/finance/splits/rules", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          rules: saveRulesPrompt.map((r) => ({
            match_value: r.description,
            category_id: r.category_id,
          })),
        }),
      });
      if (!res.ok) { toast.error("שגיאה בשמירת כללים"); return; }
      toast.success(`${saveRulesPrompt.length} כללים נשמרו — בחודש הבא הסיווג יהיה אוטומטי`);
      setSaveRulesPrompt([]);
    } finally {
  ...
  return (
    <div className="space-y-3">
      ...
      {/* ── Save rules prompt (shown after saving) ── */}
      {saveRulesPrompt.length > 0 && (
        <div className="border border-indigo-200 bg-indigo-50 rounded-xl p-3 space-y-2">
          <div className="flex items-start gap-2">
            <AlertCircle className="w-4 h-4 text-indigo-500 shrink-0 mt-0.5" />
            <div className="flex-1">
              <p className="text-xs font-semibold text-indigo-800">
                לזכור {saveRulesPrompt.length} סיווגים לעתיד?
              </p>
              <p className="text-xs text-indigo-600 mt-0.5">
                בפעם הבאה שתייבא פירוט דומה, השורות האלה יסוּוגו אוטומטית.
              </p>
            </div>
          </div>

          {/* Preview of rules to be saved */}
          <div className="bg-white rounded-lg border border-indigo-100 divide-y divide-indigo-50 max-h-28 overflow-y-auto text-xs">
            {saveRulesPrompt.slice(0, 5).map((r, i) => (
              <div key={i} className="flex items-center justify-between px-2 py-1">
                <span className="text-gray-700 truncate max-w-[160px]">{r.description}</span>
                <span className="text-indigo-600 font-medium shrink-0 mr-2">{r.category_name}</span>
              </div>
            ))}
            {saveRulesPrompt.length > 5 && (
              <p className="text-center text-gray-400 py-1">ועוד {saveRulesPrompt.length - 5}...</p>
            )}
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {savingRules ? <Loader2 className="w-3 h-3 animate-spin" /> : <Check className="w-3 h-3" />}
              כן, שמור כללים
            </button>
            <button
              onClick={() => setSaveRulesPrompt([])}
